...
@section('content')
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header d-flex align-items-center">
                    <h4 class="modal-title" id="exampleModalLabel1">Add New District</h4>
                    <button type="button" class="close ml-auto" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                </div>
                <form data-parsley-validate class="form-horizontal" method="POST" action="{{route('admin.saveDistrict')}}" id="frmSave">
                    {{ csrf_field() }}
                    ...
                    <div class="modal-footer">
                        ...
                        <input type="submit" class="btn btn-primary" id="btnSave" data-loading-text="Loading..." value="Save District"/>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="modal" id="editModal" tabindex="-1" role="dialog" aria-labelledby="Survey">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header d-flex align-items-center">
                    <h4 class="modal-title" id="exampleModalLabel1">Edit District</h4>
                    <button type="button" class="close ml-auto" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                </div>
                <form  class="form-horizontal"  action="{{route('admin.updateDistrict')}}" id="editForm">
                    {{ csrf_field() }}
                    <div class="modal-body">
...
@section('js')
    ...
    <script>
        ...
        var defaultUrl = "{{ route('admin.getDistrict') }}";
        var table;
                ...
                columns: [
                    ...
                    {data: 'name'},
                    {
                        ...
                        render: function (data, type, row) {
                            return"<button  data-url='/home/district/show/" + row.id + "' class='btn btn-info btn-sm btn-flat js-edit' data-id='" + data +
                                "' > <i class='fa fa-edit'></i>Edit</button>" +
                                "<button class='btn btn-danger btn-sm btn-flat js-delete ' data-id='" + data +
                                "' data-url='/home/district/delete/" + row.id + "'> <i class='fa fa-trash'></i>Delete</button>";
                ...
                $.ajax({
                    ...
                    if (data.district == "ok") {
                        ...
                        $('#add-messages').html('<div class="alert alert-success flat">' +
                            ...
                            '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> District  successfully saved. </div>');
            ...
            manageTable.on('click', '.js-edit', function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $("#edit-name").val(response.district.name);

                        // add hidden id
                        $('#id').val(response.district.id);
                        // update - form
...
//                                form[0].reset();
                                ...
                                $('#edit-messages').html('<div class="alert alert-success">' +
                                    '<button type="button" class="close" data-dismiss="alert">&times;</button>' +
                                    '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> District  successfully updated. </div>');
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                bootbox.alert({
                                    title: "Error",
                                    message: "<i class='fa fa-warning'></i>" +
                                        " District not Delete please try again"
                                });
                            }
                        });
